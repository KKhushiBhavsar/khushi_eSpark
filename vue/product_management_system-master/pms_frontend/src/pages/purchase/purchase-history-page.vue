<template>
  ...
  <div v-if="userPurchaseHistory?.length">
    <div class="my-10" v-for="item in userPurchaseHistory" :key="item.id">
      ...
      <v-card cols="12" v-for="product in item.products" :key="product.id">
        <!-- <v-card elevation="3" color="blue-grey-lighten-4"> -->
        <div class="d-flex flex-no-wrap justify-space-around">
          ...
          <div>
            ...
            <v-card-subtitle
              >Price: {{ getRupeeFormat(product.price) }}</v-card-subtitle
            >

            <v-card-subtitle
              >Quantity : {{ product.Cart_Product.quantity }}</v-card-subtitle
            >
          </div>
        </div>
        <!-- </v-card> -->
